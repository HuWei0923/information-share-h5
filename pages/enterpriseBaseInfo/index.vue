		...
		<view v-if="current == 0">
			<view style="display: flex;margin: 10rpx 0">
				<view class="attention-s" @click="cancleZCXFoucus" style="color: #efefef;background-color: #409eff;" v-if="careStatus.zhongchengxin == '1'">
					<uni-icons type="star-filled" size="20" class="icon-s" color="#fff"></uni-icons>
					取消关注（数据源：中诚信）
				</view>
				<view class="attention-s" @click="attentZCX" v-else>
					<uni-icons type="star" size="20" class="icon-s" color="#898989"></uni-icons>
					关注（数据源：中诚信）
				</view>
...
export default {
	...
	onShow() {
		...
		this.judgeBlackOrGray().then(() => {
			let blackText = this.isBlack ? '【黑名单】' : '';
			let grayText = this.isGray ? '【灰名单】' : '';
			this.items = [`基本信息${blackText}${grayText}`, '工商舆情'];
		});
	},
	methods: {
		judgeBlackOrGray() {
			...
			return Promise.all([userAPI.getIsBlack(param), userAPI.getIsGrey(param)]).then(res => {
				console.log(res);
				this.isBlack = res[0].data.isBlack;
				this.isGray = res[1].data.isGrey;
			});
		},
		getCareStatus(companyId) {
			...
			userAPI.getCareStatus(param).then(res => {
				if (res.data.code == '0') {
					(this.careStatus = res.data.careStatus ? JSON.parse(res.data.careStatus) : []), console.log(this.careStatus);
				}
			});
		},
		getData() {
