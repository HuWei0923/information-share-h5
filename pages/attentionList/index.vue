		...
		<view class="main">
			<view v-for="(item, index) in listData" :key="index" class="main-item" @touchend="checkDetail(item)">
				...
				<view class="main-text">
					...
					<img src="@/static/img/zcx.png" alt="" style="width: 40rpx;height: 40rpx;margin-left: 10rpx;" v-if="item.zhongchengxin == 1" />
					<img src="@/static/img/tyc.png" alt="" style="width: 40rpx;height: 40rpx;margin-left: 10rpx;" v-if="item.tianyancha == 1" />
				</view>
				<view class="tag" v-if="Number(item.messageNumber)">
					<img src="@/static/img/tag.png" alt="" style="width: 100%;height: 100%;position:absolute;left:0;top:0" />
					<text style="position: relative;top:6rpx;left:6rpx;z-index: 1">{{ Number(item.messageNumber) > 99 ? '99+' : item.messageNumber }}</text>
				</view>
			</view>
		</view>
