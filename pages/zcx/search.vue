	...
	<view class="content">
		<uni-row>
			<uni-col :span="7" style="">
				<uni-data-select style="width: 25px" v-model="selectType" :localdata="selectOptions"
					@change="changeSelectType" :clear="false"></uni-data-select>
			</uni-col>
			<uni-col :span="17">
				<uni-search-bar radius="5" placeholder="请输入搜索内容" clearButton="auto" cancelButton="none"
					@confirm="search" @clear="listData = []" v-model="keyword" />
			</uni-col>
		</uni-row>
		<view style="padding: 10rpx 20rpx; font-weight: bold">搜索历史</view>
		<view style="padding: 10rpx 20rpx; display: flex; flex-wrap: wrap">
			<uni-tag :inverted="true" :text="item.keyWord" type="primary" @click="clickHistoryTag(item)"
				v-for="item in searchHistory" :key="item.id" class="uni-mr-2 uni-mb-2" />
		</view>
		<view style="padding: 10rpx 20rpx; font-weight: bold">搜索结果</view>
		<view v-if="listData.length > 0">
			<view style="padding: 10rpx 20rpx 0">
				共搜索到
				<text style="color: #409eff; margin: 0 10rpx">{{
          listData.length
        }}</text>
				条数据结果，结果来自
			...
			<uni-group mode="card" v-for="(item, index) in listData" :key="index">
				<view slot="title" class="card-title">
					<text v-html="highLightKeyword(item.companyName)"></text>
					<uni-tag type="info" text="黑名单" v-if="allBlackList.includes(item.id)"></uni-tag>
					<uni-tag type="info" text="灰名单" v-if="allGrayList.includes(item.id)"></uni-tag>
				</view>
	...
	export default {
		...
		methods: {
			getAllBlackGrayList() {
				...
				userAPI.getAllBlackList(param).then((res) => {
					if (res.data.code == 0) {
						this.allBlackList = res.data.blackListDetailResultList.map((item) => {
							return item.tycCompanyId;
						});
						console.log(this.allBlackList);
					}
				});
				userAPI.getAllGrayList(param).then((res) => {
					if (res.data.code == 0) {
						this.allGrayList = res.data.greyList.map((item) => {
							return item.tycCompanyId;
						});
					}
				});
			...
			highLightKeyword(text) {
				...
				for (i = 0; i < len; i++) {
					...
					if (re.test(text)) {
						text = text.replace(
							re,
							`<span style="color: yellow;font-weight:bold">${this.keyword[i]}</span>`
						);
					}
				}
				return text;
			...
			fxcsIsDay(item, fxcsData) {
				...
				if (
					new Date(fxcsData.updateTime).toDateString() ===
					new Date().toDateString()
				) {
					uni.navigateTo({
						url: `/pages/zcx/historyReportList?companyId=${item.companyId}&companyName=${item.companyName}&creditCode=${item.creditCode}&pageFrom=风险初筛`,
					});
				} else {
					uni.showModal({
						title: "提示",
						...
						success: function(res1) {
							if (res1.confirm) {
								//预览pdf
								uni.navigateTo({
									url: "/pages/pdf/zcxindex?fileName=" +
										fxcsData.fileName +
										"&reportId=" +
										fxcsData.reportId +
										"&reportType=" +
