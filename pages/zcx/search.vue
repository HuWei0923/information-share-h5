      ...
      <uni-group mode="card" v-for="(item, index) in listData" :key="index">
        <view slot="title" class="card-title">
          ...
          <uni-tag
            type="info"
            text="黑名单"
            v-if="allBlackList.includes(item.id)"
          ></uni-tag>
          <uni-tag
            type="info"
            text="灰名单"
            v-if="allGrayList.includes(item.id)"
          ></uni-tag>
        </view>
        <view
          style="position: relative; line-height: 48rpx; padding-right: 60rpx"
          @click="goToPage(item)"
        >
...
<script>
import { zcxAPI, userAPI } from "api/index.js";
export default {
  data() {
    return {
      selectType: 0,
