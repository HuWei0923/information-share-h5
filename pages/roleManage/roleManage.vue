		...
		<view v-if="listData.length > 0">
			<uni-group mode="card" v-for="(item, index) in listData" :key="index">
				<view slot="title" class="card-title">
					<text style="font-weight: bold;">{{ item.roleName }}</text>
				</view>
				<view style="position: relative;line-height: 48rpx;padding-right: 60rpx;">
					<view>角色编号：{{ item.roleId }}</view>
					<view style="display: flex;">
						<view style="width: 110rpx;">权限：</view>
						<view style="display: flex;flex-wrap: wrap;flex:1">
							<uni-tag :text="item" inverted type="primary" class="uni-mr-2 uni-mb-2" v-for="(item, index) in item.permissionList" :key="index"></uni-tag>
						</view>
					</view>
